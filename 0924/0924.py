...
count = 0
for c in text:                               # 把 article 裡每個符號拿出來看，每次拿到一個符號都先把它放到變數 c 中
    if c not in "。，？；\n":   # 如果變數 c 的內容沒有出現在 "ABCDEFGHIJKLMNOPQRSTUVWXYZ" 中的話(也就是 c 不是 "A..Z" 的子字串)
        count = count + 1                       # 把 count 的值加 1 再放回 count


count = {}      # 建立一個空字典
letters = 0     # 總字元數
for c in text:                               # 把 article 裡每個符號拿出來看，每次拿到一個符號都先把它放到變數 c 中
    capc = c.capitalize()                       # 將變數 c 的內容轉大寫存在 capc 中
    if capc not in "。，？；\n":    # 如果變數 capc 的內容有出現在 "ABCDEFGHIJKLMNOPQRSTUVWXYZ" 中的話(也就是 capc 是 "A..Z" 的子字串)
        if capc not in count:                   # 如果 capc 尚未在 count 字典中
            count[capc] = 0                     # 在 count 中新增一項，把 capc 放進字典，並將其對應值設為 0
        count[capc] += 1                        # 把 count 中 capc 對應到的值加 1
        letters += 1



items = sorted(count.items(), key=lambda d: d[1], reverse=True) # 取出 count 字典中所有的項目(key, value), 排序的時候以 value 當比較的依據, 反向排序
